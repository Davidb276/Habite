...
from tienda.microservice_client import client as microservice_client, MicroserviceClientError
...
class InventarioService:
    ...
    def verificar_disponibilidad(producto_id, cantidad):
        ...
        try:
            return microservice_client.verificar_inventario(producto_id, cantidad)
        except MicroserviceClientError:
            try:
    ...
    def reducir_stock(producto_id, cantidad):
        ...
        try:
            microservice_client.reducir_stock(producto_id, cantidad)
            return True
        except MicroserviceClientError:
            try:
                inventario = Inventario.objects.get(producto_id=producto_id)
...
class PedidoService:
    ...
    def crear_pedido(self, cliente_id, productos_data, usuario=None):
        ...
        try:
            ...
            respuesta = microservice_client.crear_pedido(cliente_id, productos_data, usuario=payload_usuario)
            pedido_id = respuesta.get("pedido", {}).get("id") or respuesta.get("pedido_id")
    ...
    def actualizar_estado_pedido(pedido_id, nuevo_estado):
        ...
        try:
            microservice_client.actualizar_estado_pedido(pedido_id, nuevo_estado)
...
class PagoService:
    ...
    def procesear_pago(self, pedido_id, metodo_pago, monto):
        ...
        try:
            ...
            raise ValidationError("Pedido no encontrado")
        try:
            resultado_pago = microservice_client.procesar_pago(pedido_id, metodo_pago, monto)
            pago_id = resultado_pago.get("pago_id")
            if pago_id:
...
class EnvioService:
    ...
    def crear_envio(pedido_id, direccion_entrega):
        ...
        try:
            respuesta = microservice_client.crear_envio(pedido_id, direccion_entrega)
            envio_id = respuesta.get("envio", {}).get("id")
            if envio_id:
                return Envio.objects.get(id=envio_id)
            return Envio.objects.get(pedido_id=pedido_id)
        except MicroserviceClientError:
            try:
                pedido = Pedido.objects.get(id=pedido_id)
            except Pedido.DoesNotExist:
    ...
    def actualizar_estado_envio(envio_id, nuevo_estado):
        ...
        try:
            microservice_client.actualizar_envio(envio_id, nuevo_estado)
            return Envio.objects.get(id=envio_id)
        except MicroserviceClientError:
            try:
...
class CartService:
    ...
    def agregar_item(cliente_id, producto_id, cantidad):
        ...
        try:
            microservice_client.agregar_al_carrito(cliente_id, producto_id, cantidad)
            carrito = CartService.obtener_o_crear_carrito(cliente_id)
            return carrito.items.get(producto_id=producto_id)
        except MicroserviceClientError:
            try:
                cliente = Cliente.objects.get(id=cliente_id)
    ...
    def eliminar_item(cliente_id, producto_id):
        ...
        try:
            microservice_client.eliminar_del_carrito(cliente_id, producto_id)
            return True
        except MicroserviceClientError:
            try:
                carrito = CartService.obtener_o_crear_carrito(cliente_id)
    ...
    def vaciar_carrito(cliente_id):
        ...
        try:
            microservice_client.vaciar_carrito(cliente_id)
            return True
        except MicroserviceClientError:
            try:
                carrito = CartService.obtener_o_crear_carrito(cliente_id)
    ...
    def crear_pedido_desde_carrito(self, cliente_id, metodo_pago, direccion_entrega, usuario=None):
        ...
        try:
            respuesta = microservice_client.checkout_carrito(
                cliente_id,
                metodo_pago,
                direccion_entrega,
                usuario=payload_usuario,
            )
            pedido_id = respuesta.get("compra", {}).get("pedido", {}).get("id")
            if pedido_id:
                pedido = Pedido.objects.get(id=pedido_id)
                pago = Pago.objects.filter(pedido_id=pedido_id).first()
                envio = Envio.objects.filter(pedido_id=pedido_id).first()
                return {
                    "pedido": pedido,
                    "pago": pago,
                    "envio": envio,
                }
            raise ValidationError("No fue posible crear la compra")
        except MicroserviceClientError:
            ...
            DESCUENTO_PAGO_ADELANTADO = Decimal('0.03')
